...
document.querySelector('.btn-roll').addEventListener('click', function() {
  // (anonymous function/ can only be used here. Not named)
  if(gamePlaying) {
    // 1. Random number roll
    ...
    if (dice1 !== 1 && dice2 !== 1) {
      // Add score
      roundScore += dice1 + dice2; // roundScore = roundScore + dice;(same as typing this out)
    ...
    if (dice === 6 && lastDice === 6) {
      ...
      // Add score
      roundScore += dice; // roundScore = roundScore + dice;(same as typing this out)
      document.querySelector('#current-' + activePlayer).textContent = roundScore;
    } else {
      // Next player
      nextPlayer();
...
// using innerHTML allows you to add html elements
...
next players turn. (Hint: Always save the previous dice roll in a separate variable)
...
3. Add another dice to the game, so that there are two dice now. The player looses
